...
import { fetchEurozoneInflation, fetchMultipleCountryInflation } from "@/lib/api";
...
export function useEurozoneInflation() {
    return useQuery({
        queryKey: ["ecb", "inflation", "eurozone"],
        queryFn: async () => {
            const raw = await fetchEurozoneInflation();
...
export function useCountryInflation(countryCodes: string[]) {
    return useQuery<InflationSeriesData[]>({
        queryKey: ["ecb", "inflation", "countries", countryCodes.sort().join(",")],
        queryFn: async () => {
            if (countryCodes.length === 0) return [];

            const raw = await fetchMultipleCountryInflation(countryCodes);
...
export function useAllCountryLatestInflation(countryCodes: string[]) {
    return useQuery({
        queryKey: ["ecb", "inflation", "latest-all", countryCodes.sort().join(",")],
        queryFn: async () => {
            const raw = await fetchMultipleCountryInflation(countryCodes);
